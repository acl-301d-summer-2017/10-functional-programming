...
// TODO-DONE: Wrap the contents of this file, except for the preceding 'use strict' and 'var app...' declarations, in an IIFE.
  ...
  */
  ...
  Article.numWordsAll = () => {
    return Article.all.map(function (ele) {
      ...
      }).reduce( function(accumulator, words) {
  ...
  Article.allAuthors = (author) => {
    return Article.all.map(function (ele) {
      ...
      if (!accumulator.includes( authors)) {
        accumulator.push( authors );
      }
      return accumulator;
    }, [] );
  };
